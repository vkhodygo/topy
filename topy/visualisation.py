...
from pyvtk import CellData, Scalars, UnstructuredGrid, VtkData
...
def _fixiternum(s):
    """Fix the string by adding a zero in front if double digit number, and two zeros if single digit number."""
    if len(s) == 2:
        s = "0" + s
    elif len(s) == 1:
        s = "00" + s
